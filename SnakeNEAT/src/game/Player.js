...
class Player {
  ...
  getVision(apple) {
    ...
    if (!this.isDead(this.pos[0].copy().add(checkDirections[0])))
      this.vision[3] = 1;
    if (!this.isDead(this.pos[0].copy().add(checkDirections[1])))
      this.vision[4] = 1;
    if (!this.isDead(this.pos[0].copy().add(checkDirections[2])))
      this.vision[5] = 1;
  }
  ...
  outOfBounds(pos) {
    return pos.x < 0 || pos.x > 490 || pos.y < 0 || pos.y > 490;
  }
  ...
  isDead(pos = this.pos[0].copy()) {
    return this.steps <= 1.5 || this.outOfBounds(pos) || this.hitSelf(pos);
  }
