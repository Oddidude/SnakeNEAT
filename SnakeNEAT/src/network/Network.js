class Network {
  ...
  addNode(innovationHistory) {
    ...
    if (oldPrevNode.layer + 1 === oldNextNode.layer) {
      ...
      for (
        let newLayer = oldNextNode.layer + 1;
        newLayer < this.nodes.length;
        newLayer++
      ) {
    ...
    newNode.layer =
      Math.floor(
        Math.random() * (oldNextNode.layer - (oldPrevNode.layer + 1))
      ) +
      (oldPrevNode.layer + 1);
  ...
  randomEdge(innovationHistory) {
    if (!this.isFullyConnected()) {
      ...
      do {
        ...
        random1.layer === random2.layer ||
        random1.connectedTo(random2) ||
        random2.connectedTo(random1)
      );
  ...
  getInnovationNumber(innovationHistory, prevNode, nextNode) {
    ...
    for (let i = 0; i < innovationHistory.history.length; i++) {
      if (
        innovationHistory.history[i].checkInnovationHistory(
          this,
          prevNode,
          nextNode
        )
    ...
    innovationHistory.history.push(
      new EdgeHistory(
        prevNode.number,
        nextNode.number,
        innovationNumber,
  ...
  mutate(innovationHistory) {
    ...
    if (Math.random() <= 0.8) {
      for (let edge = 0; edge < this.edges.length; edge++)
        this.edges[edge].mutate(this.biasNode.number);
    }
  ...
  feedForward(inputVals) {
    ...
    for (let i = 0; i < inputVals.length; i++)
      this.nodes[0][i].weight += inputVals[i];
    ...
    for (let node = 0; node < outputLayer.length; node++)
      outputVals.push(outputLayer[node].input);
  ...
  crossover(otherNetwork) {
    ...
    for (let i = 0; i < this.edges.length; i++) {
      let innovationNumber = this.matchingEdge(
        otherNetwork,
        this.edges[i].innovation
      );
      ...
      if (innovationNumber !== -1) {
        ...
        if (
          !this.edges[i].enabled ||
          !otherNetwork.edges[innovationNumber].enabled
        ) {
    ...
    for (let i = 0; i < inherited.length; i++) {
      child.edges.push(
        inherited[i].clone(
          child.findNode(inherited[i].prevNode.number),
          child.findNode(inherited[i].nextNode.number)
        )
      );
  ...
  clone() {
    ...
    for (let i = 0; i < this.edges.length; i++) {
      ...
      newNetwork.edges.push(
        edge.clone(
          newNetwork.findNode(edge.prevNode.number),
          newNetwork.findNode(edge.nextNode.number)
        )
      );
  ...
  draw(x, y, w, h, size = 5) {
    ...
    for (let i = 0; i < this.nodes.length; i++) {
      ...
      for (let j = 0; j < this.nodes[i].length; j++) {
        nodePoses[i].push(
          createVector(x + i * dLayer, y + yOffset + j * dNode)
        );
      }
    }
